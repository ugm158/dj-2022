...
function get_stru_form(){
    let stru = [
      ...
      "kind":"複選",
      "type":"文字",
      "option":"選項1,選項2,選項3,選項4,選項5,選項6",
      "validate":"",
      "form_width":3,
      "default":"選項1,選項3,選項5",
      "readme":"",
  ...
  function form(e){
    ...
    let row = stru.map(function(r){
      return r.default;
    });

    // console.log(stru[6]['option']);
    // stru[6]['option'] = '選項1,選項2,選項3';
    // console.log(stru[6]['option'].split(','));
    // 轉成陣列 [ '選項1', '選項2', '選項3' ]
    // console.log(row[6]);
    // 值 '選項2,選項3'
    // console.log(row[6].split(','));
    // 值轉陣列 [ '選項2', '選項3' ]

    let option = stru[6]['option'].split(',');
    let value = row[6].split(',');
    for(let i in option){
      if(value.indexOf(option[i]) != -1){
  ...
  下午3:20:04	資訊
  下午3:20:04	資訊
  下午3:20:04	資訊	[ '選項2', '選項3' ]
     */

    // 內容
    let content = Sheet.render('form', {global: global, stru: stru, row: row});//
