function myFunction3(){
    ...
    let range,colIndex,row,rowIndex;

    row = ['王山銀行', '2022/01/20', 1000, '卡費'];
    rowIndex = 2;
    for(let i in row){
      ...
      // 寫入儲存格
      if(colIndex == 2){
        range.setNumberFormat('@').setValue(row[i]);//讓儲存格格式化成文字
      }else{
        range.setValue(row[i]);
      }
    }

    row = ['國泰銀行','2022/01/21',5000,'股票'];
    rowIndex = 3;
    for(let i in row){
      colIndex = parseInt(i) + 1;
      range = ws.getRange(rowIndex,colIndex);
